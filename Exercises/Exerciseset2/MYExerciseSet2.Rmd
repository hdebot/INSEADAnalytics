...
**Answers**
<br> <b>
1. According to the table, the first 6 factors explain ~52.29% of the the variance in this data. 
<br>
2. The weights for Comp 1, 2, and 3 are the highest weights in abslute value at respectively 14.04, 7.84, and 3.52.
<br>
3. The first 10 components can be interpreted as the 'principal components' as their eigen values are above 1. Components 11-13 also have eigen values above 1 but these values are only slightly above this threshold.
<br>
4. a. For the pre-crisis bull market the table and plot look like this:
<br>
```{r echo=FALSE}
...
b. For the financial crisis years, the figures look like this:
<br>
```{r echo=FALSE}
postcrisis_data= futures_data[as.Date(rownames(futures_data)) > "2007-10-01" & as.Date(rownames(futures_data)) < "2009-03-01" ,]
Variance_Explained_Table_results_postcrisis<-PCA(postcrisis_data, graph=FALSE)
Variance_Explained_Table_postcrisis<-Variance_Explained_Table_results_postcrisis$eig
colnames(Variance_Explained_Table_postcrisis)<-c("Eigenvalue", 
  "Pct of explained variance", "Cumulative pct of explained variance")
```

```{r}
show_data = round(Variance_Explained_Table_postcrisis, 2)
iprint.df(show_data)
```


```{r echo=TRUE}
eigenvalues_postcrisis  <- Variance_Explained_Table_postcrisis[, "Eigenvalue"]
```

```{r}
df           <- cbind(as.data.frame(eigenvalues_postcrisis), c(1:length(eigenvalues_postcrisis)), rep(1, length(eigenvalues_postcrisis)))
colnames(df) <- c("eigenvalues", "components", "abline")
iplot.df(melt(df, id="components"))
```
<br>
<br>
5. Generally the results for the bul market years and the financial crises years look similar as a relatively small number of factors is able to predict (with certain accuracy) the asset returns. In the financial crisis years however, the absolute eigen values of the largest components are relatively larger vs. the other components and it seems that an even smaller number of variables (4) is able to explain over 50% of the variance of asset returns. This suggests that for these years a smaller number of factors would have enabled us to make similar predictions as in previous years.
<br>
*
*
*
...
strategy_by_future <- scrub(shift(signal_used,5)*futures_data) # signal 2 days ago
...
**Answers**
<br>
1. The code does through the following steps:
<br> - The initial trading signal is set to 0
<br> - A sequence of 13 numbers is determined for MAfreq
<br> - For each of the 13 numbers in the MAfreq-sequence (per future), the sign is determined vs. the previous period
<br> - The relative weights of the different futures are calculated based on the total overall weight of 1
<br> - Then the trading strategy is applied: the assets are'bought' based on their relative weights. 

<br>
2. The code does through the following steps:

```{r echo=TRUE, tidy=TRUE}
signal_used_new = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq_new<-seq(10,250,by=20)
...
# Now create the returns of the strategy for each futures time series
strategy_by_future_new <- scrub(shift(signal_used_new,2)*futures_data) # signal 2 days ago
# finally, this is our futures trend following strategy
trading_strategy_new = apply(strategy_by_future_new,1,sum)
names(trading_strategy_new) <- rownames(futures_data)
```
Here is how this strategy would have performed during the given period:

```{r}
iplot.pnl(trading_strategy_new)
```


```{r}
show_data = round(pnl_matrix(trading_strategy_new),2)
iprint.df(show_data)
```
*
*
